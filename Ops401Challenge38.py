...
# Retrieves all forms on a webpage
def get_all_forms(url):
    soup = bs(requests.get(url).content, "html.parser")
    return soup.find_all("form")
...
# Submits a form with specified value and checks for XSS vulnerability
def submit_form(form_details, url, value):
    target_url = urljoin(url, form_details["action"])
    inputs = form_details["inputs"]
    data = {}
...
# dojo@dojo-VirtualBox:~/Desktop/class-38$ python3 test.py
# Enter a URL to test for XSS:https://tryhackme.com/room/owaspjuiceshop
# [+] Detected 1 forms on https://tryhackme.com/room/owaspjuiceshop.
# False
# python3 test.py
# Enter a URL to test for XSS:http://dvwa.local/vulnerabilities/xss_r/?name=%3Cscript%3Ealert%281%29%3C%2Fscript%3ERobert+Gillespe
# [+] Detected 1 forms on http://dvwa.local/vulnerabilities/xss_r/?name=%3Cscript%3Ealert%281%29%3C%2Fscript%3ERobert+Gillespie.
# False

#Note: second site is showing a false negative
#Assisted with Chat GPT